# 🤟 VRChat 한국수어교실 보조선생님 디스코드 봇: '이미숫' (Imisut Bot)
...
**이미숫 (Imisut Bot)** 은 VRChat 한국수어교실에서 **보조선생님** 역할을 맡은 디스코드 봇입니다.
...
> ⚠️ `.env.example` 의 `DEV_GUILD_ID=여기에_…` 줄은 **실제 서버 ID(숫자)로 바꾸거나 지워 주세요.** 숫자가 아닌 값이 남아 있으면 봇이 시작할 때 오류가 납니다.  
